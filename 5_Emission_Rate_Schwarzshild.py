...
for p_info in sm_content:
    ...
    #Intreal domain x>=zi
    mask = x_i >= zi
    if not np.any(mask): # If TBH is to low for this particle
        print(f"{p_info['name']:<12} | {zi:<10.2e} | {0.0:<18.2e}")
        continue
    x_int = x_i[mask]
    Rate= (27.0 * gi) / (1024.0 * np.pi**4 * G * M) * Psi(mi,s,M)
    Rate_s=Rate/(t_plank)
    print(f"{p_info['name']:<12} | {Rate_s:<18.2e}")
